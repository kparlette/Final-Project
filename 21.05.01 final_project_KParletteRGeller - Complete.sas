...
ods graphics on;
ods rtf file="/home/u58259531/EPG1V2/output/FinalProject-Complete" 
...
/*Start of Final Project - code before this is from first submission*/
/* ## Part 4: Modeling */
/*Creating Binary Variables*/
data work.movie_join2;
	set work.movie_join;

	if best_pic_nom eq 'yes' then
		best_pic_nom2=1;

	if best_pic_nom eq 'no' then
		best_pic_nom2=0;

	if best_pic_win eq 'yes' then
		best_pic_win2=1;

	if best_pic_win eq 'no' then
		best_pic_win2=0;

	if best_actor_win eq 'yes' then
		best_actor_win2=1;

	if best_actor_win eq 'no' then
		best_actor_win2=0;

	if best_dir_win eq 'yes' then
		best_dir_win2=1;

	if best_dir_win eq 'no' then
		best_dir_win2=0;

	if best_actress_win eq 'yes' then
		best_actress_win2=1;

	if best_actress_win eq 'no' then
		best_actress_win2=0;

	if top200_box eq 'yes' then
		top200_box2=1;

	if top200_box eq 'no' then
		top200_box2=0;

	if critics_rating eq 'Certified Fresh' then
		critics_rating2=2;

	if critics_rating eq 'Fresh' then
		critics_rating2=1;

	if critics_rating eq 'Rotten' then
		critics_rating2=0;

	if audience_rating eq 'Upright' then
		audience_rating2=1;

	if audience_rating eq 'Spilled' then
		audience_rating2=0;

	if mpaa_rating eq 'Unrated' then
		mpaa_ratings2=5;

	if mpaa_rating eq 'NC-17' then
		mpaa_ratings2=4;

	if mpaa_rating eq 'R' then
		mpaa_ratings2=3;

	if mpaa_rating eq 'PG-13' then
		mpaa_ratings2=2;

	if mpaa_rating eq 'PG' then
		mpaa_ratings2=1;

	if mpaa_rating eq 'G' then
		mpaa_ratings2=0;

	if genre eq 'Drama' then
		genre2=0;

	if genre eq 'Comedy' then
		genre2=1;

	if genre eq 'Horror' then
		genre2=2;

	if genre eq 'Action & Adventure' then
		genre2=3;

	if genre eq 'Animation' then
		genre2=4;

	if genre eq 'Art House & International' then
		genre2=5;

	if genre eq 'Documentary' then
		genre2=6;

	if genre eq 'Mystery & Suspense' then
		genre2=7;

	if genre eq 'Musical & Performing Arts' then
		genre2=8;

	if genre eq 'Science Fiction & Fantasy' then
		genre2=9;

	if genre eq 'Other' then
		genre2=10;

	if runtime eq 'NA' then
		delete;
	drop best_pic_nom best_pic_win best_dir_win best_actress_win best_actor_win 
		top200_box critics_rating audience_rating title_type runtime thtr_rel_year 
		thtr_rel_month thtr_rel_day mpaa_rating genre;
run;
...
proc reg data=Work.model3 alpha=0.05 plots(only)=(diagnostics residuals 
	...
	/*Model 4 - Added cross variables for those with Multicollinearity*/
...
proc glmselect data=WORK.MOVIE_JOIN2 outdesign(addinputvars)=Work.model5 
		plots=(criterionpanel);
...
proc reg data=Work.model5 alpha=0.05 plots(only)=(diagnostics residuals 
		observedbypredicted);
	ods select ParameterEstimates DiagnosticsPanel ResidualPlot 
		ObservedByPredicted;
	model Domestic_Box_Office=&_GLSMOD / vif;
...
proc reg data=Work.reg_design alpha=0.05 plots(only)=(diagnostics residuals 
	...
	title 'Model 3 Prediction';
	title2 'Sample Data to predict Revenue';

data predict3;
	format title $CHAR72.;
	input runtime2 best_pic_nom2 mpaa_ratings2 imdb_num_votes critics_score 
		top200_box2 title;
	cards;
128 0 3 266088 51 0 The_Accountant
130 0 2 416952 74 1 Fantastic_Beasts_and_Where_to_Find_Them
;
run;

data work.model3b;
	set work.model3 predict3;
	ods output "Output Statistics"=Model3P;

proc reg data=work.model3b plots=none;
	model Domestic_Box_Office=runtime2 best_pic_nom2 mpaa_ratings2 imdb_num_votes 
		critics_score top200_box2 / clm cli;
	ods exclude "Output Statistics";
	run;

proc print data=predict3;
run;

proc print data=Model3P;
	format PredictedValue comma12.0;
	format StdErrMeanPredict comma12.0;
	format LowerCLMean comma12.0;
	format UpperCLMean comma12.0;
	format LowerCL comma12.0;
	format UpperCL comma12.0;
...
data predict2;
	format title $CHAR72.;
	input runtime2 best_pic_nom2 mpaa_ratings2 imdb_num_votes audience_rating2 
		top200_box2 best_actress_win2 thtr_rel_year2 imdb_rating thtr_rel_day2 title;
	cards;
128 0 3 266088 1 0 0 2016 7.3 14 The_Accountant
130 0 2 416952 1 1 0 2016 7.3 16 Fantastic_Beasts_and_Where_to_Find_Them
;
run;

/* Prediction using Model 2 */
data work.model2b;
	set work.model2 predict2;
	ods output "Output Statistics"=Model2P;

proc reg data=work.model2b plots=none;
	format title $CHAR72.;
	model Domestic_Box_Office=runtime2 best_pic_nom2 mpaa_ratings2 imdb_num_votes 
		audience_rating2 top200_box2 best_actress_win2 thtr_rel_year2 imdb_rating 
		thtr_rel_day2 / clm cli;
	ods exclude "Output Statistics";
	run;
